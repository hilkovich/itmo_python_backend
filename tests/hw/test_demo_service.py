...
from homework_4.demo_service.api.contracts import *
...
def user_entity(user_info):
    return UserEntity(uid=1, info=user_info)


@pytest.fixture(scope="function")
def user_auth_request():
    return UserAuthRequest(username="test", password=SecretStr("123456789"))
...
def test_register_user(
    demo_servicee, user_service, register_user_request, user_response
):
    ...
    assert datetime.fromisoformat(
        response.json().get("birthdate")
    ) - user_response.birthdate < timedelta(seconds=1)
...
def test_get_user(demo_servicee, register_user_request, user_entity, user_response):
    ...
    auth_headers = {"Authorization": "Basic dGVzdDoxMjM0NTY3ODk="}
    response = demo_servicee.post(
        "/user-get",
        params={"id": first_post_resp.json().get("uid")},
    ...
    response = demo_servicee.post(
        "/user-get", params={"username": "admin"}, headers=admin_auth_headers
    )
    assert response.status_code == 200

    demo_servicee.post("/user-register", json=admin_request_data)
    response = demo_servicee.post(
        "/user-get", params={"username": "adminasd"}, headers=admin_auth_headers
    )
    assert response.status_code == 404

    auth_headers_bad = {"Authorization": "Basic YWRtaW46MTIz"}
...
def test_promote_user(demo_servicee, register_user_request, user_entity):
    ...
    auth_headers_user = {"Authorization": "Basic dGVzdDoxMjM0NTY3ODk="}
